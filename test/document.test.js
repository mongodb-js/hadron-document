...
describe('Document', function() {
  ...
  describe('#generateSetUpdateObject', () => {
    context('when nothing has been changed', () => {
      const doc = new Document({ _id: 1 });

      it('generates the appropriate document', () => {
        expect(doc.generateSetUpdateObject()).to.deep.equal({ });
      });
    });

    context('when adding a field to the document', () => {
      const doc = new Document({ _id: 1 });
      ...
      it('generates the appropriate document', () => {
        expect(doc.generateSetUpdateObject()).to.deep.equal({ name: 'test' });
      });
    });

    context('when updating a field in the document', () => {
      const doc = new Document({ _id: 1 });
    ...
    context('when updating a nested field in the document', () => {
      const doc = new Document({
        _id: 1,
        iceCream: {
          chocolate: {
      ...
      before(() => {
        const ele = doc.getChild(['iceCream', 'chocolate']);
        ele.insertEnd('cold', 'yup');
      });
    ...
    context('when an element is renamed', () => {
      const doc = new Document({ _id: 1, name: 'test' });

      before(() => {
        doc.get('name').rename('new_name');
      });

      it('it is in the generated object', () => {
        expect(doc.generateSetUpdateObject()).to.deep.equal({
          new_name: 'test'
        });
      });
    ...
    context('when an element is edited then removed', () => {
      const doc = new Document({ _id: 1, name: 'test' });
  ...
  describe('#generateUnsetUpdateObject', () => {
    context('when nothing has been changed', () => {
      const doc = new Document({ _id: 1 });

      it('generates the appropriate document', () => {
        expect(doc.generateUnsetUpdateObject()).to.deep.equal({ });
      });
    });
    ...
    context('when removing a nested field in the document', () => {
      const doc = new Document({
        _id: 1,
        iceCream: {
          chocolate: {
    ...
    context('when an element is renamed', () => {
      ...
      it('the old field key is marked to be removed', () => {
        expect(doc.generateUnsetUpdateObject()).to.deep.equal({
          name: true
        });
      });
    ...
    context('when an element is edited then removed', () => {
      ...
      it('contains the field to remove', () => {
        expect(doc.generateUnsetUpdateObject()).to.deep.equal({ name: true });
      });
    });
  });
